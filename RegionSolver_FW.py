...
class RegionSolver_FW(threading.Thread):
    """
    Run as a single thread, solve one specified region and save the result to the parameter 'regionMap'
    """

    def __init__(self, grid, boundary, xRegionMap, yRegionMap, regionMap, deMap, regionMapLock):
    ...
    def run(self):
        ...
        if self.regionMapLock.acquire() == 1:
            self.regionMap[self.xRegionMap][self.yRegionMap] = self.pathList
